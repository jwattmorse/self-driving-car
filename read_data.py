...
def read_all_images(folder_name):
    my_data = read_csv(folder_name + '/driving_log.csv',usecols = [0,1,2,3])
    my_data.columns = ['center','left','right','steering']    
    get_img = lambda file_name: img_from_file(folder_name,file_name.rsplit('/', 1)[-1])
    images = np.ndarray(shape=(my_data.shape[0],3,30,32), dtype=int)
    for i in range(my_data['center'].shape[0]):
        images[i][0] = get_img(my_data['center'][i])
        images[i][1] = get_img(my_data['left'][i])
        images[i][2] = get_img(my_data['right'][i])
    return (images,my_data['steering'].as_matrix())
...
def raw_img_from_file(folder_name,file_name):
    raw_img =  misc.imread(folder_name + '/IMG/' + file_name,mode = 'RGB')
    return raw_img
